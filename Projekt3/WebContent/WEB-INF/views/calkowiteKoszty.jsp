                            ...
                            $('tr', table).each(function (i) 
                            {
                                ...
                                $('th, td', tr).each(function (j) 
                                {
                                    if (j > 0) { // skip first column
                                        ...
                                        { // add values
                                        	 options.series[j - 1].data.push({name: sliceNames[i - 1], y: parseInt(this.innerHTML)});
                                        }
                                    }
                                });
                            });
...
select nazwa,
sum(cieplopln) as "resultcieplo", sum(energiapln) as "resultenergia", sum(gazpln) as "resultgaz", sum(wodapln) as "resultwoda" 
from obiekt
